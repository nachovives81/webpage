...
FLIPBOOK.Main = function (options, elem) {
    ...
    for (var key in skins) {
        if (this.options.skin == key) {
            this.options = FLIPBOOK.extend(true, {}, this.options, skins[key]);
        }
    }

    // eslint-disable-next-line no-redeclare
    for (var key in layouts) {
        if (Number(this.options.layout) === Number(key)) {
            this.options = FLIPBOOK.extend(true, {}, this.options, layouts[key]);
        }
    }
    ...
    // Set CSS variables on the wrapper element
    // this.wrapper.style.setProperty(this.skinBgVar, this.options.skin === 'light' ? '#f5f5f5' : '#333');
    // this.wrapper.style.setProperty(this.skinColorVar, this.options.skin === 'light' ? '#000' : '#fff');
...
FLIPBOOK.Main.prototype = {
    ...
    updateCurrentPage: function () {
        ...
        if (this.cPage.length === 2) {
            // Show the elements with class 'c-l-p' and 'c-r-p'
            ...
            // Hide the elements with class 'c-p'
            this.wrapper.querySelectorAll('.c-p').forEach(function (element) {
                element.classList.add('hidden');
            });
        } else {
            // Hide the elements with class 'c-l-p' and 'c-r-p'
            this.wrapper.querySelectorAll('.c-l-p').forEach(function (element) {
                element.classList.add('hidden');
            });
            this.wrapper.querySelectorAll('.c-r-p').forEach(function (element) {
            ...
            // Show the elements with class 'c-p'
            this.wrapper.querySelectorAll('.c-p').forEach(function (element) {
                element.classList.remove('hidden');
            });
        }
            ...
            if (!page.htmlContentInitialized) {
                ...
                if (options.pdfAutoLinks) {
                    h.querySelectorAll('span').forEach(function (span) {
                        span.innerHTML = FLIPBOOK.Linkify(span.innerHTML, options.linkTarget);
                    });
                }
    ...
    toggleIcon: function (btn, val) {
        if (btn.$iconAlt) {
            if (val) {
                btn.$iconAlt.classList.add('hidden');
                btn.$icon.classList.remove('hidden');
            } else {
                btn.$iconAlt.classList.remove('hidden');
                btn.$icon.classList.add('hidden');
            }
        } else {
    ...
    addEventListeners: function () {
        ...
        // Add all event listeners using named handlers
    ...
    createSVGIcon: function (name, reverse, transparent) {
        var icons = {
            ...
            double: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M384 80c8.8 0 16 7.2 16 16l0 320c0 8.8-7.2 16-16 16L64 432c-8.8 0-16-7.2-16-16L48 96c0-8.8 7.2-16 16-16l320 0zM64 32C28.7 32 0 60.7 0 96L0 416c0 35.3 28.7 64 64 64l320 0c35.3 0 64-28.7 64-64l0-320c0-35.3-28.7-64-64-64L64 32z"/></svg>',
            single: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><path d="M249.6 471.5c10.8 3.8 22.4-4.1 22.4-15.5l0-377.4c0-4.2-1.6-8.4-5-11C247.4 52 202.4 32 144 32C93.5 32 46.3 45.3 18.1 56.1C6.8 60.5 0 71.7 0 83.8L0 454.1c0 11.9 12.8 20.2 24.1 16.5C55.6 460.1 105.5 448 144 448c33.9 0 79 14 105.6 23.5zm76.8 0C353 462 398.1 448 432 448c38.5 0 88.4 12.1 119.9 22.6c11.3 3.8 24.1-4.6 24.1-16.5l0-370.3c0-12.1-6.8-23.3-18.1-27.6C529.7 45.3 482.5 32 432 32c-58.4 0-103.4 20-123 35.6c-3.3 2.6-5 6.8-5 11L304 456c0 11.4 11.7 19.3 22.4 15.5z"/></svg>',
    ...
    createButton: function (btn) {
        ...
        // If hover effects are required
        if (colorHover || bgColorHover) {
            var $svg = $btn.querySelector('svg');

            $btn.addEventListener('mouseenter', function () {
                $svg.style.color = colorHover;
                $svg.style.background = bgColorHover;
            });
    ...
    createMenu: function () {
        ...
        // Add additional class if conditions are met
        if (!o.menuTransparent && !o.menuBackground) {
            this.menuBottom.classList.add('skin-color-bg');
        }

        // Hide menu if option is set
        if (o.hideMenu) {
            this.menuBottom.classList.add('hidden');
        }

        // Create menuTop element
    ...
    createCurrentPage: function () {
        ...
        // Create the currentPageHolder div and append it to the menu
        ...
        // Create the form element and append it to currentPageHolder
        ...
        // Create the input element and append it to the form
        ...
        this.currentPageInput.addEventListener('focus', function () {
            ...
            // self.currentPage.classList.add('flipbook-color-transparent');
        });

        this.currentPageInput.addEventListener('blur', function () {
            self.currentPageInput.value = self.currentPageString;
            // self.currentPage.classList.remove('flipbook-color-transparent');
        });

        form.appendChild(this.currentPageInput);

        // Set the maxlength and class based on the number of digits in numPages
        ...
        // Create the currentPageNumber div and append it to currentPageHolder
    ...
    createMenuHeader: function (el, title, _) {
        ...
        var closeIcon = this.createSVGIcon('close', null, true);
    ...
    createToc: function () {
        ...
        this.tocHolder.classList.add('hidden');
        ...
        this.tocScroller = document.createElement('div');
    ...
    createTocItem: function (item, parent, level) {
        ...
        tocItem.className = 'flipbook-tocItem skin-color';
        tocItem.style.direction = rtl ? 'rtl' : 'ltr';
        ...
        tocItem.style.padding = '8px 0';
        tocItem.style['margin-' + (rtl ? 'right' : 'left')] = '10px';

        if (!level) {
            tocItem.style.marginRight = '15px';
            tocItem.style.paddingLeft = '10px';
        } else {
            tocItem.style.marginTop = '8px';
            tocItem.style.paddingBottom = '0';
        }

        var expandBtn = document.createElement('span');
        expandBtn.style.width = '20px';
        expandBtn.style.display = 'inline-block';
        expandBtn.style.cursor = 'auto';
        tocItem.appendChild(expandBtn);

        expandBtn.addEventListener('click', function (e) {
            e.stopPropagation();
            e.preventDefault();
            tocItem.items.forEach((subItem) => subItem.classList.toggle('hidden'));
            icon.style.display = icon.style.display === 'none' ? 'inline' : 'none';
            icon2.style.display = icon2.style.display === 'none' ? 'inline' : 'none';
        });

        var icon = this.createSVGIcon('next');
        ...
        var icon2 = this.createSVGIcon('next');
        expandBtn.appendChild(icon2);
        ...
        titleSpan.style.width = 150 - tocItem.level * 10 + 'px';
        ...
        if (!item.pageHidden) {
            ...
            if (pageNumber && pageNumber !== item.title) {
                ...
                pageNumberSpan.style.width = '25px';
                pageNumberSpan.style.display = 'inline-block';
                pageNumberSpan.style.textAlign = 'right';
                pageNumberSpan.textContent = pageNumber;
                tocItem.appendChild(pageNumberSpan);
            }
        }
        ...
        if (item.items && item.items.length) {
            ...
            item.items.forEach((subItem) => {
                var subTocItem = this.createTocItem(subItem, tocItem, tocItem.level + 1);
                tocItem.items.push(subTocItem);
                subTocItem.classList.add('hidden');
            });
    ...
    toggleToc: function (value) {
        ...
        if (!this.tocShowing || value) {
            ...
            this.tocHolder.classList.remove('hidden');
        } else {
            this.tocHolder.classList.add('hidden');
            this.tocShowing = false;
        }
    ...
    printFile: function (url) {
        ...
        printIframe.classList.add('hidden');
        printIframe.src = url;
    ...
    toggleSinglePage: function () {
        ...
        // var left = Number(String(this.currentPageValue).split('-')[0]);
        this.Book.updateSinglePage(this.options.singlePageMode);
        // this.goToPage(left, true);
...
FLIPBOOK.Notes.prototype = {
    ...
    showButton: function () {
        this.noteButton.classList.remove('hidden');
    },

    hideButton: function () {
        this.noteButton.classList.add('hidden');
    },
    ...
    enableNoteEdit: function () {
        this.noteDelete.classList.remove('hidden');
        this.noteInput.readOnly = false;
    },

    disableNoteEdit: function () {
        this.noteDelete.classList.add('hidden');
...
FLIPBOOK.Thumbnails = function (main) {
    ...
    this.thumbHolder.classList.add('hidden');
    ...
    this.clearInput.className = 'flipbook-search-clear hidden';

    var closeIcon = main.createSVGIcon('close', null, true);
    ...
    this.clearInput.addEventListener('click', function () {
        ...
        self.clearInput.classList.add('hidden');
    ...
    this.closeGrid.className = 'flipbook-thumbs-grid-close';
    ...
    var closeIcon = this.main.createSVGIcon('close', null, true);
    ...
    this.findInput.addEventListener('keyup', function () {
        var str = this.value;
        if (str) {
            self.clearInput.classList.remove('hidden');
        } else {
            self.clearInput.classList.add('hidden');
        }
    ...
    arr.forEach((item, i) => {
        ...
        var btnClose = document.createElement('span');
        btnClose.className = 'thumb-btn-close';
        ...
        var iconClose = main.createSVGIcon('close', null, true);
...
FLIPBOOK.Thumbnails.prototype = {
    ...
    showAllThumbs: function () {
        document.querySelectorAll('.flipbook-thumb').forEach((thumb) => thumb.classList.remove('hidden'));
        this.clearSearchResults();
    },

    hideAllThumbs: function () {
        document.querySelectorAll('.flipbook-thumb').forEach((thumb) => thumb.classList.add('hidden'));
    },
    ...
    showThumb: function (index) {
        if (this.thumbs[index]) {
            this.thumbs[index].classList.remove('hidden');
        }
    },

    hideThumb: function (index) {
        this.thumbs[index].classList.add('hidden');
    },

    showBookmarks: function () {
        document.querySelectorAll('.thumb-btn-close').forEach((btn) => btn.classList.remove('hidden'));
        this.showBookmarkedThumbs();
        this.clearSearchResults();
        this.bookmark.classList.remove('hidden');
    ...
    showSearch: function () {
        ...
        document.querySelectorAll('.thumb-btn-close').forEach((btn) => btn.classList.add('hidden'));
    ...
    show: function () {
        ...
        this.bookmark.classList.add('hidden');
        this.search.classList.add('hidden');
        this.thumbHolder.classList.remove('hidden');
...
FLIPBOOK.Lightbox = function (context, content, options) {
    ...
    // Create the overlay div and apply styles
    ...
    // Set additional background styles based on options
    ...
    // Handle close on click if enabled
    if (self.options.lightboxCloseOnClick) {
        document.body.addEventListener('click', function (e) {
            var target = e.target;
            if (
                !target.closest('.flipbook-overlay') ||
                target.classList.contains('flipbook-bookLayer') ||
                target.classList.contains('flipbook-carousel-slide')
            ) {
                self.closeLightbox();
            }
        });
    ...
    // Handle closing the lightbox on pressing the Escape key
    ...
    // Create the wrapper div and apply styles
    ...
    // Create and append the lightbox toolbar
...
FLIPBOOK.Lightbox.prototype = {
    openLightbox: function () {
        ...
        // Force a reflow to ensure the element is visible before starting the transition
    ...
    disposeLightbox: function () {
        ...
        //     let opacity = parseFloat(this.overlay.style.opacity);
        //     if (opacity > 0) {
        //         this.overlay.style.opacity = opacity - 0.05;
        //     } else {
...
FLIPBOOK.Linkify = function (element, linkTarget) {
    ...
    // Extract the text content and normalize line breaks within URLs
    let textContent = element.textContent.replace(/(\S)-\n(\S)/g, '$1$2');

    // Find all URLs and emails
    const urls = [...textContent.matchAll(urlRegex)];
    const emails = [...textContent.matchAll(emailRegex)];

    // Wrap URLs and emails in anchor tags
    urls.forEach(([url]) => wrapInAnchor(element, url, url, linkTarget));
    emails.forEach(([email]) => wrapInAnchor(element, email, `mailto:${email}`, linkTarget));

    function wrapInAnchor(element, text, href, linkTarget) {
        ...
        for (let node of textNodes) {
            ...
            // Check if node.nodeValue is equal to href or if href starts with node.nodeValue
            if (href === nodeText || href.startsWith(nodeText)) {
                const anchor = document.createElement('a');
                anchor.href = href;
                anchor.target = linkTarget;
                anchor.className = 'flipbook-page-auto-link';

                // Wrap the text node in an anchor
                node.parentNode.insertBefore(anchor, node);
                anchor.appendChild(node);
            } else if (href.includes(nodeText)) {
                // Handle case where the text node is a part of the href
                const anchor = document.createElement('a');
                anchor.href = href;
                anchor.target = linkTarget;
                anchor.className = 'flipbook-page-auto-link';
